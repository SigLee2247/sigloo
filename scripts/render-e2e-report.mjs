...
function fallbackPurpose(report) {
  if (report.driver === 'browser') return `격리된 BrowserContext에서 ${report.name ?? '브라우저 E2E'}를 실행하고 Auth Profile과 브라우저 상태 분리를 확인합니다.`;
  if (report.driver === 'process') return `기존 명령 '${report.command?.executable ?? '프로세스'}'를 실행하고 로그·artifact·정리를 확인합니다.`;
  if (report.driver === 'desktop') return 'offscreen Electron Space에서 renderer·UI·IPC·종료 정리를 확인합니다.';
  return 'Sigloo Space 실행 결과를 확인합니다.';
}
function render(report) {
  const test = report.test ?? {};
  const assertions = test.checks ?? test.assertions ?? [];
  const actions = test.actions ?? report.timeline ?? [];
